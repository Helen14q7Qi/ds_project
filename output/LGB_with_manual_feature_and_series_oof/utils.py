...
def Lgb_train_and_predict(
    train, test, config, gkf=False, aug=None, output_root="./output/", run_id=None
):
    if not run_id:
        ...
        for fold, (trn_index, val_index) in enumerate(split):
            ...
            if aug:
                ...
                trn_data = lgb.Dataset(
                    ...
                    label=train.loc[trn_index, label_name].append(
                        train_aug[label_name]
                    ),
                )
            else:
                trn_data = lgb.Dataset(
            ...
            for i in range(
                len(evals_result_dic["valid_1"][params["metric"]]) // verbose
            ):
                Write_log(
                    log,
            ...
            all_valid_metric.append(
                Metric(train.loc[val_index, label_name], valid_preds)
            )
            Write_log(
                log, "- fold%s valid metric: %.6f\n" % (fold, all_valid_metric[-1])
            )
...
def NN_train_and_predict(
    ...
    if train is not None:
        ...
        for fold, (trn_index, val_index) in enumerate(
            skf.split(train_y, train_y[label_name])
        ):
            ...
            valid_dataset = TaskDataset(
                train_series,
                train_feature,
                [train_series_idx[i] for i in val_index],
                train_y,
            )
            ...
            model = model_class(
                223, (6375 + 13) * 2, 1, 3, 128, use_series_oof=use_series_oof
            )
            scheduler = Adam12()
                    ...
                    for data in bar:
                        ...
                        if args.use_apm:
                            with amp.autocast():
                                ...
                                loss = loss_tr(
                                    outputs, y
                                )  # + loss_series # 0.5 * (loss_tr(outputs,y) + loss_feature(feature,y))
                    ...
                    if obj_max * (valid_metric) > obj_max * best_valid_metric:
                        if len(gpus) > 1:
                            torch.save(
                                ...
                                output_path + "fold%s.ckpt" % fold,
                            )
                        else:
                            torch.save(
                                model.state_dict(), output_path + "fold%s.ckpt" % fold
                            )
                        not_improve_epochs = 0
        ...
        oof.loc[oof["fold"].isin(train_folds)].to_csv(
            output_path + "oof.csv", index=False
        )
        ...
        log_df = pd.DataFrame(
            {
                "run_id": [run_id],
                "folds": folds,
                "metric": [round(mean_valid_metric, 6)],
                "lb": [np.nan],
        ...
        if not os.path.exists(output_root + "experiment_log.csv"):
            ...
            log_df.to_csv(
                output_root + "experiment_log.csv", index=False, mode="a", header=None
            )
    ...
    if test is not None:
        ...
        for fold in range(folds):
            ...
            model.cuda()
            state_dict = torch.load(
                output_path + "fold%s.ckpt" % fold, torch.device("cuda")
            )
            model.load_state_dict(state_dict)
            if len(gpus) > 1:
                model = nn.DataParallel(model, device_ids=gpus, output_device=gpus[0])
        ...
        with torch.no_grad():
            for data in tqdm(test_dataloader):
                ...
                if logit:
                    # outputs = model(data).sigmoid()
                    outputs = torch.stack([m(data).sigmoid() for m in models], 0).mean(
                        0
                    )
                    # feature,outputs = model(data)
                    # outputs = outputs.sigmoid()
